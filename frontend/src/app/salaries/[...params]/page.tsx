  ...
  useEffect(() => {
    const fetchSalaryData = async () => {
      try {
        ...
        // Validate URL format: must have exactly 4 segments (company, designation, location, uuid)
        if (segments.length !== 4) {
          setError("Invalid URL format. Expected: /salaries/company/designation/location/uuid");
          setLoading(false);
          return;
        }
        
        const [company, designation, location, salaryId] = segments;
        
        if (!salaryId) {
          setError("Salary ID not found");
          setLoading(false);
          return;
        }
        ...
        if (!uuidRegex.test(salaryId)) {
          setError("Invalid salary ID format");
          setLoading(false);
          return;
        }

        // Database entry - fetch from Supabase using ID (ID is source of truth)
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <div className="px-6 py-6">
            <CommentSection
              salaryId={salaryData.id || segments[3] || ''}
